...
def translate_with_gemini(text, target_lang="fa"):
    ...
    f"**فقط متن بازنویسی شده را برگردان و هیچ توضیح اضافی (مثل 'متن بازنویسی شده' یا موارد مشابه) اضافه نکن.**\n"
            # ---- دستورالعمل جدید برای خلاصه ----
    f"**دستورالعمل بسیار مهم: پاسخ شما باید با یک خلاصه دو خطی از کل محتوای ورودی شروع شود. این خلاصه باید دقیقاً به این شکل باشد: 'خلاصه: [خط اول خلاصه متن]\n[خط دوم خلاصه متن]'. این دو خط خلاصه باید اولین و ابتدایی‌ترین بخش از پاسخ شما باشند. بعد از این خلاصه، بقیه متن را طبق قوانین زیر بازنویسی کن.**\n"
    # ---- پایان دستورالعمل جدید ----
       
    f"قوانین مهم:\n"
    f"1. در همه جا اصول سئو کامل رعایت بشه.\n"
    f"6.در انتهای متن نتیجه گیری کن با زیر عنوان 'نتیجه گیری :  'و تو تگ  p بزارش.\n"
...
# ... (بخش اولیه پرامپت شامل دستورالعمل خلاصه و قوانین مهم ۱ تا ۱۲ باقی می‌ماند) ...
    ...
    f"ب. **ایجاد جذابیت در متن و بازآفرینی عناوین داخلی:**\n"
    f"    - **بازآفرینی خلاقانه عناوین و زیرعنوان‌ها:** اگر متن اصلی دارای زیرعنوان (تگ‌های H2, H3 و غیره) است، آن‌ها را صرفاً ترجمه نکن. بلکه آن‌ها را به شکلی خلاقانه، پرسش‌گرانه یا مبتنی بر فایده برای خواننده بازنویسی کن که کنجکاوی او را برانگیزد، مسیر مطالعه را برایش جذاب‌تر کند و به درک بهتر هر بخش کمک نماید. برای مثال، به جای 'بررسی قیمت بیت‌کوین'، می‌توانید از 'بیت‌کوین در دوراهی حساس: مقصد بعدی قیمت کجاست؟' استفاده کنید.\n"
