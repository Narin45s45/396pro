...
def translate_with_gemini(text, target_lang="fa"):
    ...
    f"For example, use 'عقب‌نشینی می‌کنه' instead of 'پس می‌کشد', or 'محدوده بریک‌اوت' instead of 'منطقه گسست'. "
    f"For titles, express the meaning naturally without forcing a specific structure (e.g., you can use a verb if it feels more natural). "
    f"If the title contains phrases like 'Shoots For' indicating a target or goal (not an achieved result), use phrases like 'به سمت' or 'هدف' instead of 'رسید'. "
    f"For example, 'XRP Price Shoots For 20% Surge To $2.51 Amid Pullback To Breakout Zone' should be conveyed as "
    f"'قیمت XRP با ۲۰٪ رشد به سمت ۲.۵۱ دلار می‌ره بعد از عقب‌نشینی به محدوده بریک‌اوت'.\n"
