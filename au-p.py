...
def translate_with_gemini(text, target_lang="fa"):
    ...
    prompt = (
    f" متن زیر یک خبر یا تحلیل در حوزه ارز دیجیتال است. من می‌خوام این متن رو به فارسی رایج بازنویسی کنی تا برای خوانندگان عمومی قابل فهم و واضح  باشه، و همزمان ارزش افزوده بده، اصول سئو کامل رعایت بشه، و شامل خلاصه‌نویسی، نتیجه‌گیری، و تحلیل عمیق بشه. لطفاً طبق قوانین زیر عمل کن:\n"
    f"**فقط متن بازنویسی شده را برگردان و هیچ توضیح اضافی (مثل 'متن بازنویسی شده' یا موارد مشابه) اضافه نکن.**\n"
    f"قوانین مهم:\n"
f"    1.خلاصه نویسی داشته باش.\n"
...
f"    5.به جای عنوان و زیر عنوان های متن از عنوان های پیشنهادی جذاب سئو استفاده کن.\n"
    ...
    f"7. محتوای متنی داخل تگ‌های HTML (مثل متن داخل <p>، <a>، یا <blockquote>) رو به فارسی رایج بازنویسی کن تا برای خوانندگان عمومی قابل فهم و واضح باشد، حتی اگر تگ‌ها ویژگی lang='en' یا هر زبان دیگه‌ای داشته باشن. این شامل محتوای متنی داخل تگ‌های تو در تو (مثل تگ‌های <p> یا <a> داخل <blockquote>) هم می‌شه.\n"
