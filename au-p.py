...
# ---- دستورالعمل جدید برای خلاصه (تاکید بیشتر بر HTML) ----
    ...
    f"    1.  محتوای آن کاملاً با استفاده از تگ‌های HTML `<b>` یا `<strong>` **بولد** شده باشد (مثال: `<b>متن بولد</b>`). **به هیچ وجه از ستاره (`**`) برای بولد کردن استفاده نکنید.**\n"
    f"    2.  داخل یک تگ پاراگراف HTML با کلاس CSS مشخص `post-summary` قرار گیرد. **دقیقاً به این صورت و با استفاده از تگ‌های HTML:** `<p class=\"post-summary\"><b>[متن خلاصه حداکثر دو خطی در اینجا]</b></p>`. خروجی باید *دقیقاً* این ساختار HTML را داشته باشد.\n"
    f"    3.  *هیچ* پیشوندی مانند 'خلاصه:' قبل از آن نداشته باشد.\n"
    f"    4.  این تگ HTML `<p class=\"post-summary\">...</p>` باید **اولین و ابتدایی‌ترین** بخش از پاسخ کامل شما باشد.\n"
    f"پس از این خلاصه، بازنویسی بقیه متن را طبق قوانین دیگر ادامه بده.**\n"
    # ---- پایان دستورالعمل جدید خلاصه ----
    f"قوانین مهم:\n"
