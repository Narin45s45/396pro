...
def translate_with_gemini(text, target_lang="fa"):
    ...
    f"**فقط متن بازنویسی شده را برگردان و هیچ توضیح اضافی (مثل 'متن بازنویسی شده' یا موارد مشابه) اضافه نکن.**\n"
           # ---- دستورالعمل جدید برای خلاصه (حداکثر ۲ خط، بولد، داخل تگ <p class='post-summary'>) ----
    ...
    f"    2.  داخل یک تگ پاراگراف با کلاس CSS مشخص `post-summary` قرار گیرد. دقیقاً به این صورت: `<p class=\"post-summary\"><b>[متن خلاصه حداکثر دو خطی در اینجا]</b></p>`.\n"
    f"    3.  *هیچ* پیشوندی مانند 'خلاصه:' قبل از آن نداشته باشد.\n"
    f"    4.  این تگ `<p class=\"post-summary\">...</p>` باید **اولین و ابتدایی‌ترین** بخش از پاسخ کامل شما باشد.\n"
    f"پس از این خلاصه، بازنویسی بقیه متن را طبق قوانین دیگر ادامه بده.**\n"
             # ---- پایان دستورالعمل جدید خلاصه ----    f"قوانین مهم:\n"
