...
def translate_with_gemini(text, target_lang="fa"):
    ...
    # ---- دستورالعمل خلاصه (با استفاده از DIV) ----
    f"**دستورالعمل بسیار مهم برای خلاصه ابتدایی:** پاسخ شما باید با یک خلاصه **کوتاه و جذاب** (حداکثر دو خط) از کل محتوای ورودی شروع شود. این خلاصه باید:\n"
    f"    1.  کاملاً **بولد (bold)** باشد (محتوای داخل تگ).\n"
    # --- تغییر از p به div ---
    f"    2.  داخل یک تگ **دیو (div)** با کلاس CSS مشخص `post-summary` قرار گیرد. دقیقاً به این صورت: `<div class=\"post-summary\"><b>[متن خلاصه حداکثر دو خطی در اینجا]</b></div>`.\n"
    f"    3.  *هیچ* پیشوندی مانند 'خلاصه:' قبل از آن نداشته باشد.\n"
    # --- تغییر از p به div ---
    f"    4.  این تگ `<div class=\"post-summary\">...</div>` باید **اولین و ابتدایی‌ترین** بخش از پاسخ کامل شما باشد.\n"
    f"پس از این خلاصه، بازنویسی بقیه متن را طبق قوانین دیگر ادامه بده.**\n"
    # ---- پایان دستورالعمل خلاصه (با DIV) ----
    ...
    f"10. هیچ تگ HTML جدیدی (مثل <p>، <b>، <div>) به متن اضافه نکن، مگر اینکه توی متن اصلی وجود داشته باشه. اگه متن اصلی تگ HTML نداره (مثلاً یه متن ساده است)، خروجی هم باید بدون تگ HTML باشه.\n"
            ...
            # --- توضیح قانون ۱۰ با توجه به خلاصه در DIV ---
    f"10. ... (توجه: برای خلاصه ابتدایی که درخواست شده، استفاده از ساختار مشخص شده `<div class=\"post-summary\"><b>...</b></div>` کاملاً مجاز است و باید طبق دستورالعمل خلاصه، در ابتدای پاسخ قرار گیرد).\n"
    # --- پایان توضیح قانون ۱۰ ---
