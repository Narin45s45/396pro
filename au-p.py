...
def translate_with_gemini(text, target_lang="fa"):
    ...
    prompt = ( f"Please translate ... English Text to Translate:\n{text}" ) # پرامپت کامل
    payload = { "contents": [{"parts": [{"text": prompt}]}], "generationConfig": {"temperature": 0.5} }
    print(f"--- ارسال برای ترجمه: '{text[:70]}...'")
    max_retries = 1; retry_delay = 3; response = None; # ... (کد کامل ارسال و دریافت و بررسی خطا مثل قبل) ...
    for attempt in range(max_retries + 1):
        try:
            response = requests.post(f"{GEMINI_API_URL}?key={GEMINI_API_KEY}", headers=headers, json=payload, timeout=90) 
            response.raise_for_status(); 
            if response.status_code == 200: break 
        except requests.exceptions.Timeout: print(f"--- خطای Timeout (تلاش {attempt+1})"); time.sleep(retry_delay); # ... (ادامه مدیریت خطا) ...
             if attempt == max_retries: raise ValueError(f"Timeout پس از {max_retries+1} تلاش")
             continue
        except requests.exceptions.RequestException as e: print(f"--- خطای شبکه (تلاش {attempt+1}): {e}"); time.sleep(retry_delay); # ... (ادامه مدیریت خطا) ...
        ...
        if response.status_code == 429 and attempt < max_retries: wait_time = retry_delay * (attempt + 2); print(f"--- خطای Rate Limit (429). انتظار {wait_time} ثانیه..."); time.sleep(wait_time) 
        elif response.status_code != 200 : 
             if attempt == max_retries : error_details = response.text; try: error_details = response.json()['error']['message']; except: pass; print(f"--- خطای نهایی API در ترجمه متن: '{text[:70]}...'"); raise ValueError(f"خطا در درخواست API (تلاش {attempt+1}): {response.status_code}, پاسخ: {error_details}")
...
# *** پردازش محتوا با BeautifulSoup (ساده‌سازی پردازش figcaption - فقط متن) ***
...
if content_source:
    ...
    try:
        ...
        for i, figcaption in enumerate(soup.find_all('figcaption')):
            ...
            # استخراج تمام متن داخل کپشن، با نادیده گرفتن تگ‌های داخلی
            original_text = figcaption.get_text(" ", strip=True)

            if original_text:
                try:
                    # ترجمه کل متن استخراج شده
                    translated_text = translate_with_gemini(original_text)

                    # پاک کردن کامل محتوای figcaption و اضافه کردن متن ترجمه شده
                    if figcaption.parent is not None:
                        ...
                        # علامت‌گذاری خود figcaption و تمام فرزندان اولیه‌اش به عنوان پردازش شده
                        processed_elements.add(figcaption)
                        for desc in figcaption.find_all(True): # پیدا کردن تمام تگ‌های فرزند (که الان باید خالی باشد)
                             processed_elements.add(desc)
                        for txt_node in figcaption.find_all(string=True): # پیدا کردن گره متنی جدید
                             processed_elements.add(txt_node)
                    ...
                    # اگر خطا رخ داد، متن اصلی (بدون تگ‌های داخلی) را نگه دار
                    if figcaption.parent is not None:
                         figcaption.clear()
        ...
        for element in soup.find_all(string=True):
            if element in processed_elements or element.find_parent(lambda tag: tag in processed_elements): continue # رد کردن اگر خودش یا والدش پردازش شده
            element_text = element.string
            if not element_text or element_text.isspace(): continue
            # ... (کد بررسی is_inside_a مثل قبل) ...
            ...
            if not is_inside_a and element.parent.name in text_parent_tags:
                ...
                # علامت‌گذاری لازم نیست چون در حلقه بعدی استفاده نمی‌شود

        for a_tag in soup.find_all('a'):
             if a_tag in processed_elements or a_tag.find_parent(lambda tag: tag in processed_elements): continue # رد کردن
             link_text = a_tag.get_text(" ", strip=True)
             if link_text:
                 a_tags_to_process.append((a_tag, link_text))
                 # علامت‌گذاری لازم نیست
        ...
        for i, (node, original_text) in enumerate(text_nodes_to_process):
            ...
            try: # ... (کد ترجمه و جایگزینی مثل قبل با time.sleep) ...
                translated_text = translate_with_gemini(original_text);
        ...
        for i, (a_tag, original_text) in enumerate(a_tags_to_process):
            ...
            try: # ... (کد ترجمه و جایگزینی لینک مثل قبل با time.sleep) ...
                translated_text = translate_with_gemini(original_text);
