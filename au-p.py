...
def translate_with_gemini(text, target_lang="fa"):
    ...
    f"2. در ابتدای محتوای اصلی (بلافاصله بعد از تگ <h1>)، متن ساده‌ای شامل زیرعنوان 'خلاصه:' و یک خلاصه مفید و کلیدی در دو خط از کل محتوای اصلی (بدون در نظر گرفتن عنوان) قرار بده. این زیرعنوان و خلاصه نباید در هیچ تگ HTML (مثل <h2>، <p> یا غیره) قرار گیرند و باید به‌صورت متن خام (plain text) باشند.\n"
    ...
    f"6. ساختار HTML موجود (مثل تگ‌های <p>، <div>، <b>، <blockquote>، <a>) را دقیقاً حفظ کن و هیچ تغییری در تگ‌ها، ویژگی‌ها (attributes) یا ترتیب آنها ایجاد نکن.\n"
    f"7. محتوای متنی داخل تگ‌های HTML (مثل متن داخل <p>، <a>، <blockquote>) را به فارسی روان بازنویسی و ترجمه کن تا ارزش افزوده داشته باشد و مفهوم را واضح بیان کند، حتی اگر تگ‌ها ویژگی lang='en' یا زبان دیگری داشته باشند. این شامل محتوای داخل تگ‌های تو در تو (مثل تگ‌های <p> یا <a> داخل <blockquote>) هم می‌شود.\n"
    f"8. هیچ تگ HTML جدیدی (مثل <p>، <b>، <div>) به متن اضافه نکن، مگر اینکه در متن اصلی وجود داشته باشد. اگر متن اصلی تگ HTML ندارد (مثلاً متن ساده است)، خروجی هم باید بدون تگ HTML باشد.\n"
    ...
    f"11. محتوای داخل تگ <h1> (عنوان اصلی) را به فارسی روان ترجمه و بازنویسی کن، اما اطمینان بده که این عنوان برای نمایش در تب مرورگر مناسب نیست و فقط در محتوای صفحه استفاده می‌شود.\n\n"
