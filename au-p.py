...
def translate_with_gemini(text, target_lang="fa"):
    ...
    f"**دستورالعمل بسیار مهم: پاسخ شما باید با یک خلاصه دو خطی از کل محتوای ورودی شروع شود.متن خلاصه باید بولد نوشته شود. این دو خط خلاصه باید اولین و ابتدایی‌ترین بخش از پاسخ شما باشند.از توضیحات اضافه مثل 'این تحلیل در مورد فلان چیز است'خوداری کن .قبل خلاصه ننویس 'خلاصه' .بعد از این خلاصه، بقیه متن را طبق قوانین زیر بازنویسی کن.**\n"
    f"قوانین مهم:\n"
    ...
    f"8. **ساختار HTML موجود (مثل تگ‌های <p>، <div>، <b>، <blockquote>، <a> و مهم‌تر از همه، تگ‌های عنوان مانند <h1>، <h2>، <h3> و غیره) رو دقیقاً حفظ کن. این شامل خود تگ‌ها، سطح‌بندی آن‌ها (یعنی H1 باید H1 باقی بماند، H2 باید H2 باقی بماند و ...)، ویژگی‌ها (attributes) و ترتیبشون می‌شه. به هیچ وجه سطح تگ‌های عنوان را تغییر نده.**\n"
    f"9. محتوای متنی داخل تگ‌های HTML (مثل متن داخل <p>، <a>، <blockquote>، <h1>، <h2> و غیره) رو به فارسی روان بازنویسی کن به طوری که ارزش افزوده پیدا کنه و مفهوم کلی رو واضح بیان کنه ، حتی اگر تگ‌ها ویژگی lang='en' یا هر زبان دیگه‌ای داشته باشن. این شامل محتوای متنی داخل تگ‌های تو در تو (مثل تگ‌های <p> یا <a> داخل <blockquote>) هم می‌شه.\n"
