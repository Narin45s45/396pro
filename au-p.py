...
def translate_with_gemini(text, target_lang="fa"):
    ...
    prompt = (
    f"متن زیر یک خبر یا تحلیل در حوزه ارز دیجیتال است که شامل عنوان (داخل تگ <h1>) و محتوای اصلی است. من می‌خوام این متن رو به فارسی روان بازنویسی کنی به طوری که ارزش افزوده پیدا کنه و مفهوم کلی را کاملا واضح بیان کنه و طبق قوانین زیر عمل کن:\n"
    f"**فقط متن بازنویسی‌شده را برگردان و هیچ توضیح اضافی (مثل 'متن بازنویسی شده' یا موارد مشابه) اضافه نکن.**\n"
    f"قوانین مهم:\n"
    f"1. اصول سئو را کامل رعایت کن، شامل استفاده از کلمات کلیدی مرتبط، ساختار مناسب سرتیترها و پاراگراف‌ها، و بهینه‌سازی برای خوانایی.\n"
    f"2. بلافاصله بعد از تگ <h1> موجود، متن ساده‌ای (بدون هیچ تگ HTML) شامل 'خلاصه:' و یک خلاصه مفید و کلیدی در دو خط از محتوای اصلی (بدون در نظر گرفتن متن داخل <h1>) قرار بده. این خلاصه باید به‌صورت متن خام (plain text) باشد و نباید در تگ‌های HTML (مثل <h2>، <p> یا غیره) قرار گیرد یا به‌عنوان سرتیتر جدید تفسیر شود.\n"
    f"3. تحلیل عمیق و ارزش افزوده به متن اضافه کن، مثل توضیحات بیشتر درباره تأثیرات موضوع، پیش‌بینی روندها، یا نکات کاربردی برای خوانندگان.\n"
    f"4. در انتهای متن، یک نتیجه‌گیری با زیرعنوان 'نتیجه‌گیری:' به‌صورت متن ساده (بدون تگ HTML) قرار بده.\n"
    f"5. در انتهای نتیجه‌گیری، کاربران را به مشارکت در نظرسنجی تشویق کن، مثلاً با پرسیدن نظرشان درباره موضوع.\n"
    f"6. ساختار HTML موجود (مثل تگ‌های <h1>، <p>، <div>، <b>، <blockquote>، <a>) را دقیقاً حفظ کن و هیچ تغییری در تگ‌ها، ویژگی‌ها (attributes) یا ترتیب آنها ایجاد نکن. به‌ویژه، هیچ تگ <h1> یا سرتیتر جدیدی اضافه نکن.\n"
    f"7. محتوای متنی داخل تگ‌های HTML (مثل متن داخل <h1>، <p>، <a>، <blockquote>) را به فارسی روان بازنویسی و ترجمه کن تا ارزش افزوده داشته باشد و مفهوم را واضح بیان کند، حتی اگر تگ‌ها ویژگی lang='en' یا زبان دیگری داشته باشند. این شامل محتوای داخل تگ‌های تو در تو (مثل تگ‌های <p> یا <a> داخل <blockquote>) هم می‌شود.\n"
    f"8. هیچ تگ HTML جدیدی (مثل <h1>، <h2>، <p>، <b>، <div>) به متن اضافه نکن، مگر اینکه در متن اصلی وجود داشته باشد. اگر متن اصلی تگ HTML ندارد (مثلاً متن ساده است)، خروجی هم باید بدون تگ HTML باشد، به‌جز خلاصه و نتیجه‌گیری که به‌صورت متن ساده اضافه می‌شوند.\n"
    f"9. Placeholderهای تصویر (مثل ##IMG_PLACEHOLDER_...##) را دقیقاً به همان شکل نگه دار و تغییر نده.\n"
    f"10. لینک‌ها (مثل آدرس‌های داخل href در تگ <a>)، نام کاربری‌ها (مثل @Steph_iscrypto)، و تاریخ‌ها (مثل May 1, 2025) را بازنویسی نکن و دقیقاً به همان شکل اصلی حفظ کن.\n"
    f"11. محتوای داخل تگ <h1> (عنوان اصلی) را به فارسی روان ترجمه و بازنویسی کن، اما اطمینان بده که فقط تگ <h1> موجود ترجمه می‌شود و هیچ تگ <h1> یا سرتیتر جدیدی اضافه نمی‌شود. این عنوان برای نمایش در تب مرورگر مناسب نیست و فقط در محتوای صفحه استفاده می‌شود.\n\n"
