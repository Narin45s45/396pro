...
def translate_with_gemini(text, target_lang="fa"):
    ...
    f"قوانین مهم:\n"
    f"13. به هیچ عنوان هیچ زیرعنوان اضافی (مثلاً تگ <h2> یا <h3> مثل 'مقدمه'، 'نگاه کلی'، یا هر چیز دیگه) قبل از تگ <p> خلاصه یا تو هر جای دیگه از متن اضافه نکن، مگر اینکه تو قوانین بعدی (مثلاً قانون ۲ برای 'تحلیل عمیق') صراحتاً ذکر شده باشه.\n"
    f"4. بلافاصله بعد از تگ <h1> (عنوان اصلی محتوا) و قبل از محتوای اصلی متن (مثلاً کپشن عکس، پاراگراف‌ها یا زیرعنوان‌های بعدی)، یک خلاصه مفید و کلیدی در دو خط قرار بده: هم زیرعنوان 'خلاصه:' و هم متن خلاصه رو تو تگ <p> قرار بده (مثلاً <p>خلاصه: دوج‌کوین داره به سمت ۱۰ دلار حرکت می‌کنه...</p>). به هیچ عنوان از تگ <h2> یا هر تگ دیگه‌ای برای زیرعنوان 'خلاصه:' استفاده نکن. همچنین، هیچ تگ، زیرعنوان، یا متنی (حتی یه پاراگراف یا زیرعنوان مقدماتی مثل 'مقدمه' یا 'نگاه کلی') قبل از این تگ <p> خلاصه اضافه نکن. این خلاصه فقط باید یه بار و دقیقاً بعد از <h1> قرار بگیره و نباید تو عنوان اصلی (<h1>) یا هر جای دیگه تکرار بشه. مثال: <h1>عنوان</h1><p>خلاصه: متن خلاصه...</p>.\n"
    f"3.تحلیل عمیق و ارزش افزوده به متن اضافه کن.\n"
    f"4.در انتهای متن نتیجه گیری کن با زیر عنوان 'نتیجه گیری :  'و تو تگ  p بزارش.\n"
    f"5.در انتها کاربران رو تحریک به نظرسنجی کن.\n"
    ...
    f"7. محتوای متنی داخل تگ‌های HTML (مثل متن داخل <p>، <a>، یا <blockquote>) رو به فارسی روان بازنویسی کن به طوری که ارزش افزوده پیدا کنه و مفهوم کلی رو واضح بیان کنه ، حتی اگر تگ‌ها ویژگی lang='en' یا هر زبان دیگه‌ای داشته باشن. این شامل محتوای متنی داخل تگ‌های تو در تو (مثل تگ‌های <p> یا <a> داخل <blockquote>) هم می‌شه.\n"
