...
def translate_with_gemini(text, target_lang="fa"):
    ...
    prompt = (
    "متن زیر یک خبر یا تحلیل در حوزه ارز دیجیتال است. من می‌خوام این PRIM رو به فارسی روان بازنویسی کنی به طوری که ارزش افزوده پیدا کنه و مفهوم کلی را کاملا واضح بیان کنه و طبق قوانین زیرعمل کن:\n"
    "**فقط متن بازنویسی شده را برگردان و هیچ توضیح اضافی (مثل 'متن بازنویسی شده' یا موارد مشابه) اضافه نکن.**\n"
    "قوانین مهم:\n"
    "1. در همه جا اصول سئو کامل رعایت بشه.\n" 
    "2.ابتدا در اولین خط متن پست(یعنی بعد از عنوان اصلی) دو خط خلاصه مفید و کلیدی از کل متن بزار با زیر عنوان 'خلاصه :'.\n"
    "3.تحلیل عمیق و ارزش افزوده به متن اضافه کن.\n"
    "4.در انتهای متن نتیجه گیری کن با زیر عنوان 'نتیجه گیری :  'و تو تگ  p بزارش.\n"
    "5.در انتها کاربران رو تحریک به نظرسنجی کن.\n"
    "6. ساختار HTML موجود (مثل تگ‌های <p>، <div>، <b>، <blockquote>، <a>) رو دقیقاً حفظ کن و تغییر نده. این شامل خود تگ‌ها، ویژگی‌ها (attributes) و ترتیبشون می‌شه.\n"
    "7. محتوای متنی داخل تگ‌های HTML (مثل متن داخل <p>، <a>، یا <blockquote>) رو به فارسی روان بازنویسی کن به طوری که ارزش افزوده پیدا کنه و مفهوم کلی رو واضح بیان کنه ، حتی اگر تگ‌ها ویژگی lang='en' یا هر زبان دیگه‌ای داشته باشن. این شامل محتوای متنی داخل تگ‌های تو در تو (مثل تگ‌های <p> یا <a> داخل <blockquote>) هم می‌شه.\n"
    "8. هیچ تگ HTML جدیدی (مثل <p>، <b>، <div>) به متن اضافه نکن، مگر اینکه توی متن اصلی وجود داشته باشه. اگه متن اصلی تگ HTML نداره (مثلاً یه متن ساده است)، خروجی هم باید بدون تگ HTML باشه.\n"
    "9. Placeholder های تصویر (مثل ##IMG_PLACEHOLDER_...##) رو دقیقاً همون‌طور که هستن نگه دار و تغییر نده.\n"
    "10. لینک‌ها (مثل آدرس‌های داخل href در تگ <a>) و متن‌های خاص مثل نام کاربری‌ها (مثل @Steph_iscrypto) یا تاریخ‌ها (مثل May 1, 2025) رو بازنویسی نکن و همون‌طور که هستن نگه دار.\n\n"
