...
def translate_with_gemini(text, target_lang="fa"):
    ...
    f"1. Convey the meaning of ALL text content, including text inside HTML tags like <p>, <li>, <blockquote>, <a>, etc. Do not skip any content.\n"
    ...
    f"4. Ensure that any text within quotation marks (\"\") conveys the same meaning in a natural way.\n"
    f"5. Preserve the original HTML structure (tags and attributes) as much as possible, only conveying the meaning of the text content within the tags and relevant attributes like 'alt' or 'title'.\n"
    f"6. Rewrite the text in colloquial and common Persian (فارسی عامیانه و رایج). Always use a casual tone, even if the initial translation sounds formal. "
    f"For example, use 'عقب‌نشینی می‌کنه' instead of 'پس می‌کشد', or 'نقطه شکست' instead of 'منطقه گسست'.\n"
    f"7. For titles, express the meaning as a noun phrase (without a verb) and use a hyphen (-) to separate the two parts of the title. For example:\n"
    ...
    f"   Persian: 'رشد ۲۰٪ قیمت XRP به ۲.۵۱ دلار- عقب‌نشینی به نقطه بریک‌اوت'\n"
