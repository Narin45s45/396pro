...
def translate_with_gemini(text, target_lang="fa"):
    ...
    f"For cryptocurrency names like 'XRP' or 'Bitcoin', always add their Persian equivalent in parentheses to clarify for readers (e.g., 'XRP(ریپل)', 'Bitcoin(بیت‌کوین)').\n"
    ...
    f"For example, use 'عقب‌نشینی می‌کنه' instead of 'پس می‌کشد', and 'محدوده شکست' instead of 'محدوده بریک‌اوت' or 'منطقه گسست'. "
    f"For titles, follow this structure: first mention the context or event (e.g., a pullback, dip, or other market movement), then describe the price movement (e.g., surge, rise, or target), and end the sentence with an exclamation mark (!) to add emphasis. "
    ...
    f"   Persian: 'قیمت بیت‌کوین(بی‌تی‌سی) بعد از افت به سطح حمایت با رشد ۱۰٪ به سمت ۷۰,۰۰۰ دلار می‌رود!'\n"
