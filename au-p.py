...
def translate_with_gemini(text, target_lang="fa"):
    ...
    f"**دستورالعمل بسیار مهم: پاسخ شما باید با یک خلاصه دو خطی از کل محتوای ورودی شروع شود. متن خلاصه باید با تگ <strong> بولد شود، داخل تگ <p> قرار گیرد و کل خلاصه در تگ <div class=\"summary\"> باشد تا برای استفاده در تلگرام قابل شناسایی باشد. قبل از خلاصه هیچ عبارت اضافی مانند 'خلاصه' یا توضیحات مشابه درج نشود. بعد از این خلاصه، بقیه متن را طبق قوانین زیر بازنویسی کنید.**\n"        
    f"قوانین مهم:\n"
    f"1. در همه جا اصول سئو کامل رعایت بشه.\n"
    f"2.6. در انتهای متن نتیجه‌گیری کن با زیرعنوان 'جمع‌بندی:' و در تگ <h4 class="conclusion"> قرار بده.\n"
    f"3. محتوای متنی داخل تگ‌های HTML (مثل متن داخل <p>، <a>، یا <blockquote>) رو به فارسی روان بازنویسی کن به طوری که ارزش افزوده پیدا کنه و مفهوم کلی رو واضح بیان کنه ، حتی اگر تگ‌ها ویژگی lang='en' یا هر زبان دیگه‌ای داشته باشن. این شامل محتوای متنی داخل تگ‌های تو در تو (مثل تگ‌های <p> یا <a> داخل <blockquote>) هم می‌شه.\n"
    f"4. اصطلاحات 'bear'، 'bearish' یا مشابه را به 'فروشندگان' یا 'نزولی' (بسته به زمینه) و اصطلاحات 'bull'، 'bullish' یا مشابه را به 'خریداران' یا 'صعودی' (بسته به زمینه) ترجمه کن. از کلمات 'خرس' یا 'گاو' به هیچ عنوان استفاده نکن.\n"
    f"5. تاریخ‌های میلادی (مانند May 1, 2025) را به فرمت شمسی (مانند ۱۱ اردیبهشت ۱۴۰۴) تبدیل کن. تاریخ‌ها باید دقیق و مطابق تقویم شمسی باشند و به صورت متنی (نه عددی مثل 1404/02/11) نوشته شوند. اگر تاریخ در متن مبهم است (مثلاً فقط ماه و سال)، فقط ماه و سال را به شمسی تبدیل کن.\n"
    f"6. ساختار HTML موجود (مثل تگ‌های <p>، <div>، <b>، <blockquote>، <a>) رو دقیقاً حفظ کن و تغییر نده. این شامل خود تگ‌ها، ویژگی‌ها (attributes) و ترتیبشون می‌شه.\n"
    f"7.در انتها کاربران رو تحریک به نظرسنجی کن.\n"
    f"8. هیچ تگ HTML جدیدی (مثل <p>، <b>، <div>) به متن اضافه نکن، مگر اینکه توی متن اصلی وجود داشته باشه. اگه متن اصلی تگ HTML نداره (مثلاً یه متن ساده است)، خروجی هم باید بدون تگ HTML باشه.\n"
    f"9. Placeholder های تصویر (مثل ##IMG_PLACEHOLDER_...##) رو دقیقاً همون‌طور که هستن نگه دار و تغییر نده.\n"
    f"10. لینک‌ها (مثل آدرس‌های داخل href در تگ <a>) و متن‌های خاص مثل نام کاربری‌ها (مثل @Steph_iscrypto) یا تاریخ‌ها (مثل May 1, 2025) رو بازنویسی نکن و همون‌طور که هستن نگه دار.\n\n"
