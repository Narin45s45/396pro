...
# *** پردازش محتوا با BeautifulSoup (بازگشت به ترجمه تک به تک ساده) ***
...
if content_source:
    ...
    try:
        ...
        # 3. استخراج گره‌ها و متن‌ها (بدون منطق پیچیده کپشن P)
        ...
        # تگ‌های والد مجاز برای متن‌های معمولی (شامل figcaption)
        ...
        for element in soup.find_all(string=True):
            ...
            # بررسی اینکه داخل لینک نباشد
            is_inside_a = False; curr = element.parent
            while curr:
                if curr.name == 'a': is_inside_a = True; break
                curr = curr.parent
            
            # اگر والد مجاز است و داخل لینک نیست -> افزودن به لیست متن معمولی
            if not is_inside_a and element.parent.name in text_parent_tags:
                # *** لاگ کردن متن معمولی پیدا شده (شامل متن کپشن اگر والدش figcaption باشد) ***
                print(f"--- DEBUG: افزودن متن معمولی (والد: {element.parent.name}): '{element_text[:50]}...'")
        ...
        for a_tag in soup.find_all('a'):
            ...
            if link_text:
                 print(f"--- DEBUG: افزودن متن لینک: '{link_text[:50]}...'")
        ...
        for i, (node, original_text) in enumerate(text_nodes_to_process):
            current_segment += 1
            is_caption_node = node.parent.name == 'figcaption' # بررسی والد
            log_prefix = f"--- پردازش کپشن" if is_caption_node else f"--- پردازش متن معمولی"
            print(f"{log_prefix} {current_segment}/{total_segments}...")

            try:
                translated_text = translate_with_gemini(original_text)
                if node.parent is None: print(f"--- هشدار: گره {current_segment} والد ندارد!"); continue
                if hasattr(node, 'replace_with'):
                    node.replace_with(NavigableString(translated_text))
                    successful_text_translations += 1
                    # *** لاگ کردن جایگزینی موفق کپشن ***
                    if is_caption_node: print(f"--- DEBUG: متن کپشن با موفقیت جایگزین شد: '{translated_text[:50]}...'")
                else: print(f"--- هشدار: گره {current_segment} خاصیت replace_with ندارد.");
                time.sleep(0.2)
            except Exception as e:
                print(f"--- خطا در ترجمه/جایگزینی متن {current_segment} ('{original_text[:30]}...'): {e}. استفاده از متن اصلی.")
                time.sleep(0.5)
        ...
        for i, (a_tag, original_text) in enumerate(a_tags_to_process):
            ...
            try:
                ...
                if a_tag.parent is not None:
                     ...
                     # *** لاگ کردن جایگزینی موفق لینک کپشن ***
                     # (نیاز به بررسی دارد که آیا این لینک داخل کپشن بوده یا نه)
                     # is_inside_caption = False; curr = a_tag.parent ... (کد بررسی والد)
                     # if is_inside_caption: print(f"--- DEBUG: متن لینک کپشن جایگزین شد: '{translated_text[:50]}...'")
        ...
        for img in soup.find_all('img'):
            ...
            # اضافه کردن استایل به figcaption اگر وجود دارد
            figcaption = img.find_next_sibling('figcaption')
            if figcaption:
