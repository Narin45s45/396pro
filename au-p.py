...
def translate_with_gemini(text, target_lang="fa"):
    ...
    prompt = (
    f" متن زیر یک خبر یا تحلیل در حوزه ارز دیجیتال است. من می‌خوام این متن رو به فارسی روان بازنویسی کنی به طوری که ارزش افزوده پیدا کنه و مفهوم کلی را واضح بیان کنه و طبق قوانین زیر عمل کن:\n"
...
f"    2.به جای عنوان و زیر عنوان ها از عنوان های جذاب و کلیدی استفاده کن.\n"
f"    3.در ابتدای متن 2 خط خلاصه از کل متن داشته باش بدون هیچ زیر عنوانی.\n"
f"    4.تحلیل عمیق و ارزش افزوده به متن اضفه کن.\n"
f"    5.در انتهای متن نتیجه گیری کن با زیر عنوان 'نتیجه گیری :'.\n"
f"    6.در انتها کاربران رو تحریک به نظرسنجی کن.\n"
    f"7. ساختار HTML موجود (مثل تگ‌های <p>، <div>، <b>، <blockquote>، <a>) رو دقیقاً حفظ کن و تغییر نده. این شامل خود تگ‌ها، ویژگی‌ها (attributes) و ترتیبشون می‌شه.\n"
    f"8. محتوای متنی داخل تگ‌های HTML (مثل متن داخل <p>، <a>، یا <blockquote>) رو به فارسی روان بازنویسی کن به طوری که ارزش افزوده پیدا کنه و مفهوم کلی رو واضح بیان کنه ، حتی اگر تگ‌ها ویژگی lang='en' یا هر زبان دیگه‌ای داشته باشن. این شامل محتوای متنی داخل تگ‌های تو در تو (مثل تگ‌های <p> یا <a> داخل <blockquote>) هم می‌شه.\n"
    f"9. هیچ تگ HTML جدیدی (مثل <p>، <b>، <div>) به متن اضافه نکن، مگر اینکه توی متن اصلی وجود داشته باشه. اگه متن اصلی تگ HTML نداره (مثلاً یه متن ساده است)، خروجی هم باید بدون تگ HTML باشه.\n"
    f"10. Placeholder های تصویر (مثل ##IMG_PLACEHOLDER_...##) رو دقیقاً همون‌طور که هستن نگه دار و تغییر نده.\n"
    f"11. لینک‌ها (مثل آدرس‌های داخل href در تگ <a>) و متن‌های خاص مثل نام کاربری‌ها (مثل @Steph_iscrypto) یا تاریخ‌ها (مثل May 1, 2025) رو بازنویسی نکن و همون‌طور که هستن نگه دار.\n\n"
